      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="px-4 pb-3 font-mono text-[12.5px] leading-[1.6]">
          {panel === "Terminal" && (
            <>
              {terminalLines.map((l) => (
                <div key={l.id} className={cn("whitespace-pre-wrap", LINE_CLASS[l.kind])}>
                  {l.text}
                </div>
              ))}
              <div className="mt-3 font-semibold text-[var(--code-fn)]">relay-session</div>
          ...
          {panel === "Problems" && (
            <div className="text-[var(--code-number)]">
              src/parser/guard.ts:41:12 — nested import guard may recurse infinitely
            </div>
          )}
          {panel === "Output" && (
            <div className="text-muted-foreground">[14:08:02] Sync complete — 0 conflicts</div>
          )}
          {panel === "Debug Console" && (
            <div className="text-muted-foreground">Debugger not attached.</div>
          )}
          {panel === "Ports" && (
            <div className="text-muted-foreground">8080 → https://localhost:8080 (forwarded)</div>
          )}
        </div>
      </ScrollArea>
    </div>
