...
import { chatEmotes, chatMessages, type ChatBadge, type ChatMessage, type Participant } from "@/lib/mock-data";
...
> = {
  ...
  guest: { label: "Guest", icon: IconStar, className: "bg-secondary text-muted-foreground" },
  bot: { label: "Bot", icon: IconRobot, className: "bg-secondary text-muted-foreground" },
};
  ...
  return body.split(/(@[\w]+)/g).map((part, i) =>
    part.startsWith("@") ? (
      <span
        ...
        style={{ color: `var(${colorVar})`, backgroundColor: `color-mix(in oklch, var(${colorVar}) 18%, transparent)` }}
      >
        {part}
      </span>
    ) : (
  ...
  return (
    <div
      className={cn(
        ...
        m.highlighted && "bg-primary/10 ring-1 ring-inset ring-primary/30 hover:bg-primary/15",
      )}
    >
      {m.replyTo && (
      ...
      <p className="text-sm leading-relaxed">
        ...
        <span className="break-words text-foreground/90">{renderBody(m.body, colorVar)}</span>
      </p>
      ...
      <div className="absolute right-1 top-1 hidden items-center gap-0.5 rounded-md border border-border bg-popover p-0.5 shadow-sm group-hover:flex">
        ...
        <span className="px-1 text-[10px] tabular-nums text-muted-foreground">{m.time}</span>
      </div>
    </div>
  );
}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className={cn("truncate text-sm font-medium", idle && "text-muted-foreground")}>
            {p.name}
          </span>
          {p.isYou && (
        ...
        <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
          ...
          <span className={cn(!idle && "text-success/80")}>{idle ? "Idle" : "Active"}</span>
          {p.sharing && (
            <>
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {p.cameraOn ? (
          <IconVideo className="h-4 w-4 text-muted-foreground/70" aria-label="Camera on" />
        ) : (
          <IconVideoOff className="h-4 w-4 text-muted-foreground/40" aria-label="Camera off" />
        )}
        {p.micOn ? (
          <IconMicrophone
            className={cn("h-4 w-4", p.speaking ? "text-success" : "text-muted-foreground/70")}
            aria-label="Microphone on"
          />
        ) : (
          <IconMicrophoneOff className="h-4 w-4 text-destructive" aria-label="Muted" />
        )}
      </div>
    </div>
  );
    ...
    <Tabs value={tab} onValueChange={onTabChange} className="flex h-full flex-col gap-0">
      <div className="border-b border-border p-3">
        <TabsList className="grid h-8 w-full grid-cols-2 gap-1 rounded-md border border-border bg-background p-1">
          ...
          >
            People
            <span className="ml-1.5 text-xs text-muted-foreground">{participants.length}</span>
          </TabsTrigger>
          <TabsTrigger
            value="chat"
            className="h-6 rounded-[5px] text-sm data-[state=active]:bg-secondary data-[state=active]:text-foreground data-[state=active]:shadow-none"
        ...
        <ScrollArea className="h-full">
          <div className="flex flex-col p-2">
            {participants.length === 0 ? (
              <p className="p-4 text-xs text-muted-foreground">No participants in this room.</p>
            ) : (
              <>
                <div className="flex items-center justify-between px-2.5 pb-1.5 pt-1">
                  ...
                  <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                    ...
                    {participants.filter((p) => p.presence === "active").length} active
                  </span>
                </div>
      ...
      <TabsContent value="chat" className="mt-0 flex min-h-0 flex-1 flex-col">
          ...
          <div className="flex flex-col gap-1 p-2">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center gap-2 px-4 py-10 text-center">
                <IconMessage2 className="h-5 w-5 text-muted-foreground" aria-hidden />
        ...
        <div className="shrink-0 border-t border-border">
            ...
            {chatEmotes.map((e) => (
              <Tooltip key={e.name}>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    onClick={() => setDraft((d) => (d ? `${d} ${e.char}` : e.char))}
          ...
          >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button type="submit" size="icon" className="h-8 w-8" disabled={!draft.trim()}>
