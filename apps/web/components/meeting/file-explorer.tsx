...
export function FileExplorer({
   ...
   const [creating, setCreating] = useState<{ folderId: string; kind: "file" } | null>(
      null
   )
   ...
   const filtered = useMemo(() => {
      ...
      return tree
         .map(f => ({
            ...f,
            files: f.files.filter(n => n.name.toLowerCase().includes(q)),
         }))
      ...
      setTree(t =>
         t.map(f =>
            f.id === id
               ...
               : {
                    ...f,
                    files: f.files.map(n => (n.id === id ? { ...n, name: value } : n)),
                 }
         )
      )
   }
   ...
   function duplicateNode(folderId: string, node: ExplorerNode) {
      ...
      setTree(t =>
         t.map(f =>
            f.id === folderId
               ? { ...f, files: [...f.files, { id: nextId(), name: copy }] }
               : f
         )
      )
   }
   ...
   function commitNewFile(folderId: string, name: string) {
      ...
      setTree(t =>
         t.map(f =>
            f.id === folderId
               ? { ...f, files: [...f.files, { id: nextId(), name: value }] }
               : f
         )
      )
   }
                        ...
                        <ContextMenu>
                           <ContextMenuTrigger asChild>
                              <div
                                 ...
                                 onDragLeave={() =>
                                    setDropFolder(f => (f === folder.id ? null : f))
                                 }
                                 onDrop={e => {
                                    e.preventDefault()
                              ...
                              {folder.files.map(node => {
                                 ...
                                 if (renaming === node.id) {
                                    return (
                                       <div
                                          key={node.id}
                                          className="mx-1 py-0.5 pl-7 pr-1.5"
                                       >
                                          <input
                                             autoFocus
                                             value={draft}
